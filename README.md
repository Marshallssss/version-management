...
### 2026-09-22 发布时间维护与项目选择

- 基线“历史维护”编辑的是发布时间，实际创建时间和审计发生时间不改。草稿不能通过改时间冒充已发布；三分钟撤回仍从真实发布操作计时，补录日期不会重开期限。
- 项目选择弹窗加宽、选择框占满一行；下拉菜单统一浅色主题、选中态和禁用态。现代 Edge/Chrome 使用同主题选项面板，旧浏览器保留原生选项兼容。
- 无项目标准的新项目不再误报读取失败，正常显示“尚未设定”。项目下拉的视觉与键盘验收见 [项目选择复核](docs/ui/project-picker-review.md)。
- 本次新增 Migration `20260922031634_BaselineWithdrawalUsesReleaseActionTime`，其他部署环境须先备份并执行现有数据库升级流程，再启动 Host/Worker。PostgreSQL 17 / Windows Production Integration Pending 验收边界不变。
- 历史基线 Excel 目前是设计方案，尚未开放导入；建议同一模板明确区分测试版本和完整历史基线，后者必须管理员手动预览确认。字段、校验、扫描隔离与待确认边界见 [历史基线导入设计](docs/historical-baseline-import-design.md)。

### 2026-09-20 项目资料与批量上机

- 导入会核对当前项目、所选模板、工作簿绑定和组件标识；其他项目的文件不能因组件同名而误导入。
- 顶部项目选择弹窗可“编辑项目资料”，修改编码、名称和说明；克隆项目时可单独填写新说明，不影响来源项目。
- 基线历史由管理员通过“编辑基线名称”修改名称并填写原因；版本快照仍冻结。日期筛选、排序和详情使用真实“发布时间”，未发布记录不伪造时间。
- “Lab 使用与验证”支持多选测试版本、多选 Lab 整机或腔室。每个范围单独保存，失败后仅重试失败项；登记的是已完成的实际操作，不是远程安装。
- 机台创建、克隆、编辑和详情增加“工艺（非必填）”“配置（非必填）”。配置是文字资料，不替代组件版本实际配置。
- **本次需要升级数据库**：先备份，执行现有升级流程应用 `20260920022058_AddMachineProcessConfigurationAndBaselineRename`，再启动 Host/Worker。不要只替换前端文件。PostgreSQL 17 已验证，Windows Production Integration Pending 边界不变。


### 版本导入与批量机台登记

- 左侧“版本导入”按“生成模板 → 填写版本 → 上传并录入 → 每日扫描（可选）”提供首次使用引导。先确认首次导入结果，再由管理员配置固定文件扫描。
- 机台页右上角新增“Excel 导入机台”：下载当前项目模板，一行一台，填写资料、整机/PM 阶段后先检查再确认。工艺、配置非必填；已有序列号不覆盖。
- 机台模板只识别当前项目已发布基线，生成初始实际配置，不设置目标、不冒充安装记录。未知基线仍允许导入资料，软件版本留空并以红色警告显示，近期记录可追溯。
- 本片无新 Migration。填写格式、校验及失败重试说明见 [机台 Excel 导入](docs/machine-excel-import.md)。
...
- 基线历史支持按发布时间正序/倒序、日期区间、多组件具体版本筛选；多项条件须同时满足，日期按本地日历且包含结束日。组件和版本选项读取冻结快照，历史不随当前资料改名；“项目标准”标签只表示项目推荐，不会修改机台目标。
