...
- 扫描记录逐组件显示“名称 从旧版本变为新版本”，点击定位对应测试组合和组件；组合展示前后版本并以浅红背景醒目标记。旧历史只读补充差异，不重写快照；超过最近列表范围的组合仍可按链接单独打开。
- Admin/SuperAdmin 可以删除仅有 Excel 引用的误登记版本，解除关联外键但保留原始版本 ID/名称、组合快照和审计；重复扫描已导入记录不会复活版本，新组合如沿用已删除版本须明确填入正确版本。基线、机台与 Lab 等事实引用继续保护。
- 已废弃版本仅允许管理员恢复测试，记录原因、审计和新测试轮次，并将同组件旧测试版废弃；Safety 不跟随成熟度改变。接口在幂等重放前重新检查管理员权限。
- 实验室树将使用状态改为“未上机 / 已上机”，与“测试中”同高、右侧深色标记，包含在原节点内；未知加载状态不冒充未上机，详情仍保留独立验证结论。版本页删除入口同步管理员权限。
- 新增 `matrix-template-v2`、`import-admin-correction`、`matrix-diff-ui` 自动验收；前者覆盖 14 类错误/不完整输入、文本版本号、整批事务、冻结继承、重复扫描与已录入列保护，新旧模板均完成真实 Worker 到时扫描。纠错验收覆盖降权后的幂等重放拒绝、仅 Excel 引用删除、历史保留、后续继承已删版本拦截、恢复测试及基线/机台/风险保护；页面覆盖旧组合独立查询、跨项目隔离、错误重试和精确定位。
- Release build 0 warning / 0 error、前端 build 无大包警告、真实 Migration 检查与 EF pending model changes 均通过；全部 29 组页面/API/解析回归通过，另通过 catalog、background-job、Windows operations preflight（13 脚本）、web compatibility。Host/Worker 已运行，live/ready 为 200。
- 独立 XLSX 读取器确认 `001.02` 保持文本、日期说明、根分区、隐藏绑定及冻结窗格；渲染复核布局，渲染器会把纯数字样式文本显示成数字，实际原文件及导入值已独立验证不丢前导零，未冒充原生 Excel 人工验收。桌面/390px 页面截图已复核并保存，Lab 标记在节点内部且不增加行高。
- README 与 Excel/Lab 使用说明同步新版列式录入、取消提交标记、旧模板兼容及管理员纠错。新模板重新下载即可；本片无新 Migration，不更改已执行 Migration，PostgreSQL 17 / Windows Production Integration Pending 边界不变。

### 2026-09-18 项目矩阵 Excel 与 Lab 验证闭环

- 用户确认矩阵模板：按当前项目组件树、名称和排序生成分组表头；组件为列，每行是一套测试组合，填写表示变更，空白继承。分类节点仅分组，有版本意义的根组件保留版本列。模板冻结初始项目标准，后续行继承前一成功组合；不动态追随标准，不把空白解释为删除。
- 版本登记表仅需记录日期、整行说明和可导入标记；新版本只能进入 Testing，已有版本引用不改生命周期，重复扫描不重复创建。已导入记录改写报冲突，失败前序不允许后续错误继承；保存组合完整快照和原始来源。导入复用 UI 的版本业务命令，保留权限、reason、actor、correlation、幂等与事务。
- 交付顺序：模板下载/上传扫描/组合查看；固定路径每日扫描与手动触发、错误恢复；Lab 整机/PM 关联、实际升级记录与验证结果。扫描由服务器 Worker 执行，路径由管理员配置，文件稳定后读取，不回写用户正在编辑的工作簿，不运行宏或外部链接。
- Lab 升级是记录已完成的实际操作，不是自动安装软件；只更新选定组件，不改变其他实际配置、机台 Target 或项目标准。版本成熟度、机台使用及验证结论独立，保留升级时间和观察时间的区别，机台离开 Lab 不删除历史证据。发布检查统一覆盖状态发布、初始发布、测试组合生成基线和超级维护发布；兼容开关默认关闭，开启后要求本轮至少一项通过且各范围无未解决失败。没有实现逐次发布豁免，不将配置关闭冒充审计豁免。
- 已实现 Excel 生成/上传录入/完整组合快照、每日固定路径 Worker 扫描与手动扫描、中文错误及失败恢复。已导入行不可覆盖；文件读取异常五分钟后重试，内容错误等待修正和下一次扫描。模板及组合引用保护组件/版本删除，删除影响预览同步列出 Excel 和 Lab 引用。
- 已实现版本树 Lab 整机/PM 数量和验证状态标记、右侧使用/验证面板、机台跳转、实际升级和验证记录；新版本或状态改变同步刷新标记。PM 局部升级不清空其他特例，实际升级时间必须明确输入；新一轮测试不能复用上一轮通过记录。
- Lab 面板显示发布验证策略是否强制；失败后登记“待验证”不会抹掉未解决失败，须同范围复测通过，汇总标记与发布校验使用同一规则。真实页面验收明确构造快照早于升级的时间线，不用回填升级覆盖更晚的配置事实。
- 页面实际保存回归发现分钟精度可能早于同分钟内开始的测试轮次；升级/验证输入增加秒精度，接口明确拒绝早于本轮测试开始的验证时间，避免成功保存却不计入本轮的误导。
- 矩阵验收补充跨行事务：前一新行已经保存新版本并废弃旧测试版本，后一行无效时两者均恢复；另覆盖待填写前序阻断后续录入、非模板列拒绝，避免静默跳过内容。
- 由真实 EF 模型生成并已应用 `20260917145620_ProjectMatrixImportAndLaboratoryValidation`，不修改旧 Migration；Release 0 warning / 0 error、EF 无待迁移模型变更、前端无大包警告。catalog/background-job、Windows operations preflight（13 脚本）、web compatibility 已通过，既有 24 组页面/API 回归及新增 matrix-import/laboratory 均通过；真实 Worker 到时扫描、权限撤销后拒绝重放、删除影响预览和真实页面验证保存已验证。修正后的 Lab 发布门槛与删除影响 API/UI 再次回归通过。
- README 与 `docs/excel-matrix-laboratory.md` 说明模板填写、固定路径授权、Worker 运行、发布开关及部署升级；既有粘贴导入保留，但新建版本也统一为测试中，需项目高级工程师权限。没有增加 NuGet/npm 依赖。
- 桌面/390px 实页截图已复核并保留到 `docs/ui/screenshots`；该记录是开发侧视觉检查，不替代真实用户验收。导入和 Lab 面板沿用主干浅色风格，自动扫描执行人不暴露内部编号。独立 XLSX 读取器确认 `001.02` 文本、隐藏绑定、冻结窗格和下拉结构，未宣称原生 Excel 人工兼容验收。
- 本片范围：仅识别本系统生成的 `.xlsx`，一项目一个扫描源；单模板 2000 行、496 个版本组件、10 MiB。最近 100 次扫描/200 套组合在页面展示，历史仍保留；任意第三方 Excel 自动识别、分页查询、批量 Lab 升级和单次发布豁免不在本片。PostgreSQL 17 / Windows Production Integration Pending 边界不变。

### 2026-09-16 机台检索、独立基线与项目总览

- 按正式使用反馈整理信息架构：左侧“项目”更名“版本”，独立“基线”页承接冻结快照、筛选、比较、项目标准与既有维护/撤回流程；测试版本创建基线、搜索和受保护引用跨页跳转，补丁入口回到版本详情。
- 机台默认一机一行，显示名称/序列、位置、机型/阶段/状态、负责人、PM、目标基线、整机实际版本及独立 Match/Risk。支持站点/位置、状态/阶段、目标基线、实际组件版本和关注项筛选；“战场”暂按站点/位置理解，兼容状态/阶段需求。实际版本不从目标基线推断。
- 基线历史按录入时间排序，当前项目标准在列表与详情标记；支持本地日期范围和多组件具体版本 AND 筛选。单次只读索引使用冻结名称/版本，不受后续改名影响；上一条比较使用完整时间序列，不随筛选或显示排序改变。
- 运行总览改为当前项目配置关注入口：严重风险、配置差异、信息不足、无目标，点击进入机台筛选；提供当前标准、测试版本与近期基线。明确配置记录不等于在线监控，风险与匹配独立，无数据/加载失败不冒充健康。管理员系统运维收至总览折叠栏目，按需查询服务与全系统后台任务。
- 查询沿用既有登录/只读授权，写操作仍走原领域命令；新 API 批量查询，不逐机台/逐基线请求详情，无模型或 Migration 变更。PostgreSQL 17 / Windows Production Integration Pending 边界保持不变。
- 日期范围验收发现历史维护接收带时区日期时未转 UTC，导致 PostgreSQL 写入失败；保存前统一转 UTC，保持同一时间点和精度，界面仍按本地日期筛选。补齐机台筛选控件的明确可访问名称，不改变现有筛选语义。
- 2026-09-17 验收完成：新增 machine-registry、baseline-history-filter、navigation-overview 自动验收通过；合计 21 组 UI/API 回归通过，覆盖基线/补丁跨页引用、项目隔离、Matched + Critical、只读权限、超级维护关闭、导入及机台/PM 事实保护。更新既有验收的导航定位，保留原业务断言。截图复核后修正总览及筛选条的原生按钮样式；补齐基线窄屏完整页面截图，外部基线跳转清理旧筛选。
- 全解决方案 Release 0 warning / 0 error、前端构建无大包警告、真实 Migration 已最新、EF 无模型差异；catalog/background-job、Windows operations preflight（13 脚本）、web compatibility 通过。Host/Worker 本地运行，设计和截图证据见 docs/ui/workspace-navigation-review.md；代码与文档一并提交主干。
...
- 方向：浅铂灰工作区、石墨导航、克制蓝/青强调，测试使用独立琥珀色、风险使用红色。中文可读性、紧凑层级与真实操作效率优先；统一导航、表单、弹窗、项目树、机台、比较及查询页面。
- 分片顺序：共享视觉与导航；项目/基线/版本检查器及机台/PM/比较；实际数据、多根节点、长名称、空状态、Viewer 与桌面/窄屏验收。截图逐张检查，不以构建通过代替视觉验收。
- 完成状态：本片实现、开发侧视觉检查与自动验收完成，设计记录和截图随分支交付；等待用户预览确认后再决定是否合并主分支。PostgreSQL 17 与 Windows Production Integration Pending 不变。
- 首轮实页检查后修正窄检查器按钮竖排、顶部项目栏旧样式覆盖与图标干扰无障碍名称；机台匹配/风险分别着色，用户列表增加筛选并将新增/调权收进弹窗。保留领域命令与权限判断。
- 本片实现并验收：统一全局主题、中文角色标签、导航图标、顶部项目栏、项目/基线/检查器、机台/PM/比较、搜索/导入/部署/运维及用户管理。新增 future-ui-acceptance 覆盖真实登录、9 页、35 张截图、用户创建/角色变更真实保存、Viewer、冻结快照/事实不变；六根长名称及既有 14 组 UI/API 回归通过。截图发现的问题已返修，设计侧检查不冒充真实用户主观验收。
- Release 0 warning / 0 error、真实 Migration 已最新、EF 无模型变更、前端构建无大包警告；catalog/background-job、Windows operations preflight（13 脚本）、web compatibility 通过。Host/Worker 已重启；README 与 docs/ui/future-ui-review.md 同步设计、验收和合并边界。仅提交推送独立分支，主分支保持不变。
- 末轮补齐搜索结果行的原生边框清理与项目/当前项目组件上下文，同号版本更容易区分；统一批量复选框尺寸、主要基线创建动作及输入焦点颜色。未新增查询权限或跨项目详情请求。重启后的新版验收与维护关闭真实验收通过，live/ready 为 Healthy。

### 2026-09-13 目标与历史补丁入口

- 第五片 D 已验收：复用补丁浮窗补齐机台目标、项目标准、时间点实际及当前/历史比较，预期与实际分列展示，允许切换只看差异/全部组件。匹配与风险继续分开；缺少版本记录不擅自推断为已确认缺失。
- 目标基线名称沿用冻结快照；历史实际按事实版本 ID 重建并使用现有接口名称，补丁浮窗明确属于当前资料，不代表当时补丁清单或已经安装。查询和跳转不改事实、安装时间、基线内容；本片不变更模型或 Migration。
- 机台事实录入后同步刷新时间点实际与当前/历史比较缓存，防止停留在旧查询结果。共用对比表保留加载/失败/空结果状态；Viewer 的时间查询和浮窗是只读操作。
- 新增 history-patches-acceptance 已通过：预期/实际及历史两侧跳转、当前三类补丁状态、无补丁/明确缺失不产生假入口、历史安装时间未知、仅名称维护后基线标签保持冻结、读取前后事实/快照不变、Matched + Critical 与 Viewer UI/API 权限；1366/768/390 截图已检查。
- README 同步使用说明。Release 0 warning / 0 error、前端构建通过；真实 Migration 已最新，EF 无模型差异；catalog/background-job、Windows operations preflight（13 个脚本）、web compatibility，以及既有补丁/PM/项目/机台/维护/布局/权限回归均通过。重启后的新验收再次通过，Host/Worker 运行正常，live/ready 均 Healthy。
- 剩余：按项目与机台样板继续整理搜索、导入及部署记录等页面的视觉和中文术语；旧整机比较接口的 Unknown/Absent 精细语义不在本片冒进修改。当前调测维护设置、PostgreSQL 17 与 Windows Production Integration Pending 边界不变。

### 2026-09-12 删除与维护操作确认

- 第六片 A 已验收：版本删除/维护与基线撤回/历史维护增加只读影响预览，展示受保护引用、关联记录及可跳转名称，每组准确计数并最多预览 10 条。已删除机台、已移除 PM 的历史仍受保护；读取失败或仍在加载时不能确认。提交仍由原事务、权限、幂等与领域规则重新判断，预览通过不是写入授权。
- 维护确认区分两种语义：版本维护保留冻结基线；基线历史维护直接修正选中快照，并提示项目标准/机台目标/历史展示可能受影响，机台实际事实不重写。取消保留输入，保存等待刷新完成后再关闭，修复连续维护可能沿用旧选择的问题。
- 时间回归修正：历史维护按本地时区显示录入时间；版本与基线未编辑日期时保留原秒数及精度，仅明确修改日期才转换写入。真实 UI 测试覆盖仅改名称/成熟度、仅换组件版本与单独修改录入时间。
- 维护关闭真实验收通过：独立 Host 与临时配置验证 SuperAdmin 新请求和关闭前成功请求的幂等重放均被拒绝，Viewer/匿名也无权限，UI 不显示维护入口；旧版本/快照不变，正常登记仍可用。原配置与当前维护开关不变，临时进程/目录已清理。Host/Worker 可选 CONFIGHUB_LOCAL_CONFIG_PATH 路径覆盖，默认读取位置不变，显式路径缺失时停止启动。README 已同步操作与验收说明。
- 新增 operation-impact-acceptance / operation-impact-ui-acceptance / maintenance-closed-acceptance 均通过；可选 --expiry 也真实等待发布后 180 秒验证预览与撤回命令同时拒绝，快照和审计不变。覆盖权限、只读查询、受保护引用、清理计数、预览后新增引用再次阻断、确认/取消/跳转与 1366/390 弹窗截图。原 record-maintenance 同步确认步骤并通过。
- 全解决方案 Release 0 warning / 0 error；真实 Migration 已最新、EF 无模型差异，前端构建无大包警告。catalog/background-job、Windows operations preflight（13 个脚本）、web compatibility 以及既有项目/机台/PM/补丁/导入/布局/权限回归通过，Host/Worker 已重新运行。本轮无新增 Migration，未改既有 Migration。
- 故障注入补齐：版本维护、基线维护与撤回在已有缓存的情况下遇到预览失败，仍禁止确认；失败和刷新恢复期间零写请求且记录不变，恢复后须显式确认。新增断言后的整套 UI 验收再次通过。
- 验证边界：TopComponentVersionId 与 SupersedesBaselineId 的孤立历史引用已纳入查询，但当前 API 不自然生成此类独立记录，本轮未直接构造数据库分支验收。仍使用本机 PostgreSQL 17，不将 Windows Production Integration Pending 宣告完成。
- 下一片：补齐旧式目标差异摘要/历史配置表的补丁入口，再按现有项目与机台样板推广其他页面；超级维护保持当前设置，正式冻结仍由用户明确决定。
...
- 第三片 A 已验收：版本与补丁列表优先，登记按钮按需展开表单；取消后同对象可继续填写，校验失败保留输入，成功关闭表单，切组件/版本清理不适用的草稿。补丁保存同时刷新项目树标记；Viewer 保持只读。新增 project-entry-acceptance 覆盖取消重开、重复版本错误、真实登记版本/补丁、PM 影响跳转。
...
- 剩余工作更新见 2026-09-12：影响预览与维护关闭验收已完成；旧式目标差异摘要/历史配置表的补丁入口、其他页面视觉推广仍待处理。没有关闭调测维护，Windows Production Integration Pending 仍保留。
...
- 组件增加负责人、型号、备注，创建/编辑/克隆/API/审计贯通，使用新生成的 EF Migration。
- 实验室节点改为名称、版本两行；基线选择父节点全选后代测试版本，支持半选与取消选择。
- 允许 Released → Testing，保留状态历史及单组件单测试版本约束；已发布基线快照不随版本状态回退而修改。
- 测试版本“发布并创建基线”在一次数据库事务中发布所选版本并冻结基线，不再逐个点击送审/通过/发布；保留发布权限、阻断校验、幂等、审计与三分钟撤回。项目标准和机台目标仍须显式指定。
- 核验 SuperAdmin 单版本删除入口与接口，保留基线/机台/PM 历史引用保护。
- 已发布版本返回测试时撤销推荐，旧草稿如包含返回测试的版本则禁止发布；父节点勾选包含所有后代，叶子只保留版本勾选框，避免重复操作。
- 直接发布锁定项目及候选版本后校验，在一个事务中先写入快照再冻结；不伪造独立评审通过记录，审计标记 DirectPublish；拒绝勾选发布但未选入快照的测试版本。
- 当前项目标准快照读取完成前禁止提交，避免尚未加载的默认版本被最新发布版本替代；实验室名称/版本两行加入浏览器几何断言。
- 真实 EF Migration `20260909080639_AddComponentMetadata` 已生成并应用，无模型差异。新增 catalog-simplification-acceptance 覆盖空格编码查重/克隆、点分补丁搜索、组件资料 UI/API/克隆、重返测试、分支全选/半选、直接发布/阻断回滚/并发幂等、旧快照及旧机台目标保留。
- 已通过 catalog、workspace-ux、project-scope-ui、version-delete、Windows operations preflight、web compatibility；桌面/手机布局检查完成，前后端 Release 构建通过。保留原有 PostgreSQL 17 / Production Integration Pending 生产部署验收边界。
...
- 当前项目改为顶栏内无常驻边框的名称与简介，增加呼吸空间并移除重复账户文字；实验室组件名称与版本同行显示，长内容保留悬停全文，测试计数压缩为同行标签。
- 实验室历史增加组件筛选；基线历史根列使用当前组件排序展示，历史父子关系和冻结快照数据不变。
- Viewer 隐藏项目/版本/补丁/基线/机台写入表单及相应入口，保留历史、筛选、对比等读取操作；API 权限校验仍独立生效。
- 机台列表、创建、复制、批量升级与批量目标统一使用顶部当前项目，不再出现重复项目选择。配置比对统一为双侧机台/基线选择器，支持机台↔机台、机台↔已发布基线、基线↔基线、交换左右、只看差异；Match 与 Risk 独立展示，不把未评估风险显示为无风险。
- 修正未设置项目标准时空响应转为明确 null，避免查询层将 undefined 当作错误并反复显示正在读取。
- 验证：前端构建通过；project-scope-ui 自动化验收覆盖组件历史筛选、根排序与快照不变、项目隔离、Viewer 表单隐藏/API 403、四种对比方向及桌面/手机无横向溢出。既有 workspace 浏览器回归通过。Release build 零警告/零错误，真实 Migration 执行确认已是最新，EF 无待生成模型差异；catalog、record-maintenance、workspace、project-scope-ui 回归均通过，Windows preflight 13 个脚本与 web compatibility 通过。前端仍有既有资源体积提示。
...
4. **1D 批量 Deployment/Observation（进行中）**：批量仅编排既有事实命令，不能绕过版本、组件、FULL/PARTIAL、Audit 或幂等规则。
   - **1D-0 后台 Job 状态机（已交付）**：真实 EF Migration 已增加 `last_attempt_at`，并将既有 `Processing/Completed` 前进为 `Running/Succeeded`；状态机固定为 Pending → Running → Succeeded，失败时 Running → Retry → Running，达到最大次数后 → Failed。管理员运行总览显示等待重试、上次尝试时间和新状态中文文本。Host 启动前会检查 EF Pending Migrations；数据库落后于二进制时直接停止启动并明确要求以 Migration 连接串运行 `ConfigHub.Host.exe --migrate`，避免运行总览持续报缺失字段。`background-job-acceptance.ps1` 在真实 Host/Worker/PostgreSQL 上验证成功任务的 `Succeeded`、尝试/完成时间，以及无处理器任务的 `Running → Retry`、错误与清理；目录验收、Release build、真实 Migration 均已回归通过。
...
7. **3A Exposure 与可导出追溯（已交付）**：真实 EF Migration 已创建 `version_exposure_snapshots`、`version_exposure_machines`、`version_exposure_baselines`；版本进入 Blocked 时在同一事务固化 Current、Target、Historical Machine 与 Baseline 范围，并写入独立 Audit，解除 Blocked 不回写历史快照。版本影响中文页已展示快照计数与阻断上下文；`catalog-acceptance.ps1` 已验证阻断快照的 Current/Historical 范围和独立审计。Version Impact CSV 已从版本“影响”页交付：工程师必须填写导出原因，浏览器下载 UTF-8 CSV，后端仍负责项目范围授权、关联 ID、Idempotency 内容重放和导出 Audit；自动验收覆盖 CSV 内容与审计事件。Release build、真实 Migration 与 EF Pending Model Changes 均在本片交付前复核通过。
...
- 2026-09-07 项目与机台 UX 修正（已交付）：优先处理用户本轮反馈，暂停导入映射扩展。恢复导航 15px 字号并缩窄侧栏；统一收合、维护、排序与机台表单的控件样式。项目默认收起详情，点击组件自动展开；排序按需显示。实验室提供统一历史弹窗与唯一创建入口，跳转基线历史按根/子层级选版。草稿取消不设倒计时；正式发布后 3 分钟内可撤回为 Deprecated，保留冻结快照和审计，已被标准、机台或后续修订使用的基线拒绝撤回，真实前进 Migration 补充数据库保护。管理员删除需二次确认，允许同步删除无历史引用的组件版本、补丁和生命周期；基线、机台事实及阻断快照引用保持保护。实际验收：Release build 0 warning / 0 error、真实 PostgreSQL 17 Migration、EF 无 Pending Model Changes、目录验收（包含数据库直接约束、发布后 2 分钟允许/4 分钟拒绝、快照不可夹带修改、管理员权限与版本同步删除）、Web 兼容、13 个 Windows 运维脚本预检通过。新增 `workspace-ux.cjs` 在 Edge 验证默认收合/点击展开、排序显隐、8 个测试版本的统一历史、父子选版、删除二次确认的取消/确认，以及 1440px/390px 项目和机台布局无横向溢出；截图位于本地 `artifacts/workspace-ux`。机台操作收束为标签区域，弹窗采用现有浅色主题，并移除隐藏的重复项目工作区。
...
- 3L 组件结构可恢复性切片已交付：根组件可在同级中前移或后移，服务端以交换 `sort_order` 的命令、RBAC、Idempotency 与 Audit 持久化顺序；不恢复无效虚线框，而是允许把组件拖到“当前基线版本”标题区域，直接还原为根组件，原有拖到组件卡片改变父节点和防循环校验保持不变。
- 3M 基线编排可用性切片已交付：实验室树只保留测试状态总览，创建入口移至基线历史顶部的“基线选项”面板；工程师在同一处勾选 Testing 版本、按需修正其余 Released 版本并填写原因。基线名称默认采用排序第一根组件的当前候选版本号。基线快照改为浅蓝紧凑网格，降低长树在历史页的纵向占用，同时保留缩进表达父子层级。
- 3N 组件总览与测试状态约束已交付：当前基线根组件改为响应式多列总览，使用可记忆的列宽密度控制在可用宽度内优先完整展示；右侧已选组件面板可收起，为大项目释放横向空间。根组件排序的视觉方向与横向布局一致；当前基线仅保留根节点计数，组件名称与版本以不同字重和颜色表达。服务端将保证同一组件在任一时刻最多一个 `Testing` 版本：登记或切换新的测试版本时，旧测试版本必须在同一事务中转为 `Deprecated`，并写入 Lifecycle 与 Audit；PostgreSQL Migration 会先修复历史重复行并记录系统迁移审计，再以部分唯一索引强制该不变量。基线草稿取消不再限时，可恢复的自动发布版本会回到 `Testing`；正式发布后提供 3 分钟内的可审计撤回窗口，撤回为 `Deprecated` 并保留冻结快照，已被标准、机台或后续修订使用的基线拒绝撤回。真实 Migration 与 `catalog-acceptance.ps1` 已覆盖替换测试版本和快速撤回后恢复测试状态。
- 3O 版本可见性与基线变更追溯已交付：实验室测试树为每个 Testing 版本提供时间线入口，打开后显示从开始测试到发布、废弃或安全状态变化的时间线；组件总览在当前基线版本存在补丁时显示标记和可收起的简短预览。基线历史会把所选快照与时间线上紧邻的上一份快照进行对比，先给出变化数量，再按组件展示新增、移除或新旧版本，避免工程师在两个长快照之间肉眼查找。项目 Detail 投影仅携带最近三条补丁摘要用于预览，完整问题与修复记录仍由版本详情 API 提供；Release build 与 `catalog-acceptance.ps1` 已通过。
...
- 4E 历史维护模式已交付：只有 `SuperAdmin` 可以在明确展开“历史维护”后，为 Draft、已评审或已发布 Baseline 补录 2000–2100 年间的录入时间和已发布版本；命令必须带 reason、Idempotency-Key、maintenanceMode 标志与 Audit，并在其单一数据库事务内设置受限 PostgreSQL 开关以临时通过不可变 Trigger。普通 API、普通 Admin 及数据库直连写入仍被拒绝；完成历史补录后应撤销执行者的 `SuperAdmin` 权限，以恢复严格冻结。项目“删除”采用可审计归档，日常项目列表只显示 Active 项目，历史数据不物理删除。
- 身份初始化补强：常规启动先查询 Identity 数据库；已有 Admin 时不会读取或重置引导账户配置。只有显式 `--bootstrap-admin-only` 流程才会创建、验证或按 `BootstrapAdmin:ResetPassword` 重置引导账户。
...
- 5F 完整实际配置工作台已交付：机台实际配置拆分为安全的“单组件局部观察”和“完整配置工作台”。完整工作台会显式列出项目全部可配置组件，可从已发布基线加载版本作为编辑起点，也可加载当前实际配置；用户再按组件选择已发布或 Testing 版本/标记缺失，并一次性写入完整事实与用户输入的生效时间。单组件入口固定为 `PARTIAL`，不能以单组件输入伪造 FULL。基线模板不自动成为升级来源或 Machine Target，Testing 版本仍不可进入 Baseline 或 Target；多机基线升级继续使用 5D 的来源基线审计入口。`catalog-acceptance.ps1` 覆盖实验室机台的 Testing 版本完整配置、所有组件投影、生效时间、无伪造来源基线与幂等重放；Release build、真实 Migration、SPA build 与 EF Pending Model Changes 均已通过。
- 5G 机台版本直达已交付：机台当前实际配置中的版本现在可直接打开所属项目的既有版本详情工作台，复用版本状态、补丁、时间线与影响面，而不复制一套机台专用版本页。跳转会切换并记住所属项目、保留版本焦点并清除无关基线焦点；缺失版本保持只读文本。SPA build 验证组件和路由类型，`catalog-acceptance.ps1` 继续覆盖版本详情、补丁、影响面和机台实际配置的真实 API 链路；Release build、真实 Migration 与 EF Pending Model Changes 均已通过。
...
- 10D GitHub Windows Release 自动化已交付：新增本地与 CI 共用的 `package-release.ps1`，在发布 Host/Worker `win-x64` EXE 后复核 manifest 每个文件、生成 ZIP 与整包 SHA-256，并验证 ZIP 内容与 manifest 一致；同版本包拒绝覆盖；Windows 运维预检已纳入该脚本的必需文件与 PowerShell 语法检查。新增只可手动触发的 `.github/workflows/release.yml`，在 GitHub Windows Runner 上执行 Release build、SPA build、Web 兼容、Windows 运维预检和 EF Pending Model Changes，再上传同一份包并创建 `v<version>` GitHub Release。EF 模型检查使用不连接数据库的设计时 `ConfigHubDbContext` Factory，因此不读取生产连接串、不启动 Host，也不把 GitHub Runner 伪装为 PostgreSQL 集成环境。工作流默认使用公共源，但可通过 Actions Variables/Secrets 配置公司 NuGet/npm 镜像及令牌；严格内网使用已预热的 Windows 自托管 Runner 与离线构建模式，秘密和连接串永不进入仓库或发布包。2026-09-02 首次云端预发布发现 YAML 中嵌套 PowerShell here-string 会被缩进破坏，已改为数组组装 NuGet 凭据 XML；随后将所有 GitHub 布尔输入改为字符串比较，避免 PowerShell 把裸 `false` 当作失败命令。已以本地离线 `win-x64` 资产实际发布 `0.2.0-pilot.2` 临时 smoke 包，Host/Worker EXE、manifest、ZIP 与整包 SHA-256 均通过校验。完整中文操作说明见 `docs/operations/github-release.md`。
