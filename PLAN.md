...
- 5B 进行中：Machine Target Assignment 将保存独立有效区间、actor 和 reason；不在 Machine 保存 Target 指针，也不由 Project Standard 自动创建 Assignment。当前 API 已限制同项目 Released Baseline，自动验收已覆盖显式指派；待补完整幂等和中文界面。
