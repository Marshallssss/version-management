...
6. **2B 比较能力（进行中）**：已交付同项目 Machine vs Machine 的实时 Current Actual 比对与中文界面；Match（版本差异）和 Risk（Blocked 版本风险）分别计算。`catalog-acceptance.ps1` 覆盖 `Matched + None` 与同版本被 Blocked 后的 `Matched + Critical`。下一片为 Machine Current vs Historical，继续不扩展为万能 Compare。
