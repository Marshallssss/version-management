...
if (@($factHistory | Where-Object { $_.operationType -eq 'InitialSnapshot' -and $_.coverage -eq 'Full' }).Count -ne 1 -or @($factHistory | Where-Object { $_.operationType -eq 'Observation' -and $_.coverage -eq 'Partial' }).Count -ne 3 -or @($factHistory | Where-Object { $_.operationType -eq 'Observation' -and $_.coverage -eq 'Full' }).Count -ne 1 -or @($factHistory | Where-Object { $_.operationType -eq 'Rollback' -and $_.coverage -eq 'Partial' }).Count -ne 1 -or @($factHistory | Where-Object { $_.sourceType -eq 'agent-automation' }).Count -ne 1) { throw 'Machine fact history must retain operation, coverage, rollback and external event semantics.' }
