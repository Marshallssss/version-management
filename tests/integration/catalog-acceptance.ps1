...
if ($baselineList.Count -ne 3 -or @($baselineList | Where-Object { $_.state -eq 'Released' }).Count -ne 2 -or @($baselineList | Where-Object { $_.id -eq $explicitBaseline.id -and $_.state -eq 'Draft' }).Count -ne 1) { throw 'Expected released baseline history plus the explicit draft composition.' }
