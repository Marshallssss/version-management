...
if ($baselineDetail.baseline.id -ne $baseline.id -or $baselineDetail.baseline.state -ne 'Draft' -or $baselineDetail.items.Count -ne 2 -or @($baselineDetail.items | Where-Object { $_.componentCode -eq 'CONTROL' -and $_.componentName -eq 'Control' }).Count -ne 1) { throw 'Baseline detail must expose the frozen component tree snapshot.' }
...
$fullFacts = @{ operationType = 'InitialSnapshot'; coverage = 'Full'; sourceType = 'automation'; reason = '自动化完整快照'; items = @(@{ componentId = $component.id; versionId = $secondVersion.id; absent = $false; knownInstalledAt = $knownInstalledAt }, @{ componentId = $child.id; versionId = $childVersion.id; absent = $false; knownInstalledAt = $null }) } | ConvertTo-Json -Depth 5
...
if ($actual.Count -ne 2 -or $actualComponent.Count -ne 1 -or $null -eq $actualComponent[0].knownInstalledAt -or (([DateTimeOffset]$actualComponent[0].knownInstalledAt).ToUniversalTime() - $knownInstalledAt.ToUniversalTime()).Duration().TotalSeconds -gt 1 -or @($actual | Where-Object { $_.componentId -eq $child.id -and $_.versionId -eq $childVersion.id -and $_.state -eq 'Present' }).Count -ne 1) { throw 'Partial observation must preserve unobserved current components and must not turn observation time into installed time.' }
...
if (@($factHistory | Where-Object { $_.operationType -eq 'InitialSnapshot' -and $_.coverage -eq 'Full' }).Count -ne 1 -or @($factHistory | Where-Object { $_.operationType -eq 'Observation' -and $_.coverage -eq 'Partial' }).Count -ne 1 -or @($factHistory | Where-Object { $_.operationType -eq 'Observation' -and $_.coverage -eq 'Full' }).Count -ne 1) { throw 'Machine fact history must retain operation and coverage semantics.' }
