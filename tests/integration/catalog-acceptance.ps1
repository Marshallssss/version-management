...
$engineerJob = Invoke-RestMethod -WebSession $viewerSession -Method Post -Uri ([uri]::new($BaseUri, '/api/v1/system/jobs/noop')) -Headers @{ 'Idempotency-Key' = [Guid]::NewGuid().ToString() } -ContentType 'application/json' -Body (@{ reason = '自动化工程师后台任务许可' } | ConvertTo-Json)
if ($null -eq $engineerJob.id) { throw 'Engineer system job enqueue must succeed.' }
