...
$draftBeforeMaintenance = Invoke-RestMethod -WebSession $session -Uri ([uri]::new($BaseUri, "/api/v1/baselines/$($labBaseline.id)"))
$maintenanceBody = @{ versionSelections = @(@{ componentId = $component.id; versionId = $thirdVersion.id }); reason = '自动化维护模式构造历史'; maintenanceMode = $true } | ConvertTo-Json -Depth 5
...
if ($maintainedDetail.baseline.createdAt -ne $draftBeforeMaintenance.baseline.createdAt -or $null -ne $maintainedDetail.baseline.releasedAt -or @($maintainedDetail.items | Where-Object { $_.componentId -eq $component.id -and $_.versionId -eq $thirdVersion.id }).Count -ne 1) { throw 'Maintenance mode must only update a draft baseline with explicit audit-friendly historical data.' }
...
$releasedBeforeMaintenance = Invoke-RestMethod -WebSession $session -Uri ([uri]::new($BaseUri, "/api/v1/baselines/$($baseline.id)"))
$releasedMaintenanceBody = @{ releasedAt = '2002-03-04T05:06:00+00:00'; versionSelections = @(@{ componentId = $component.id; versionId = $secondVersion.id }); reason = '自动化已发布历史补录'; maintenanceMode = $true } | ConvertTo-Json -Depth 5
...
if (([DateTimeOffset]$releasedMaintenance.baseline.releasedAt).Year -ne 2002 -or $releasedMaintenance.baseline.createdAt -ne $releasedBeforeMaintenance.baseline.createdAt -or $releasedMaintenance.baseline.releaseRecordedAt -ne $releasedBeforeMaintenance.baseline.releaseRecordedAt -or @($releasedMaintenance.items | Where-Object { $_.componentId -eq $component.id -and $_.versionId -eq $secondVersion.id }).Count -ne 1) { throw 'SuperAdmin historical maintenance must update an already released baseline through the guarded transaction.' }
...
$testingActualVersion = Invoke-JsonPost "/api/v1/components/$($component.id)/versions" @{ versionNumber = 'opaque-workbench-test'; reason = '自动化完整配置工作台测试版本' }
Invoke-Lifecycle "/api/v1/component-versions/$($testingActualVersion.id)/maturity" 'Testing' '自动化完整配置工作台进入测试' | Out-Null
$fullWorkbenchMachine = Invoke-RestMethod -WebSession $session -Method Post -Uri ([uri]::new($BaseUri, '/api/v1/machines')) -Headers @{ 'Idempotency-Key' = [Guid]::NewGuid().ToString() } -ContentType 'application/json' -Body (@{ projectId = $project.id; serialNumber = "FULL-WORKBENCH-$suffix"; name = 'Full configuration workbench machine'; machineType = 'Laboratory'; reason = '自动化完整配置工作台机台' } | ConvertTo-Json)
$fullWorkbenchAt = [DateTimeOffset]::UtcNow.AddMinutes(2)
$fullWorkbenchHeaders = @{ 'Idempotency-Key' = [Guid]::NewGuid().ToString() }
$fullWorkbenchBody = @{ operationType = 'InitialSnapshot'; coverage = 'Full'; sourceType = 'manual-ui'; effectiveAt = $fullWorkbenchAt; reason = '自动化完整配置工作台验收'; items = @(@{ componentId = $component.id; versionId = $testingActualVersion.id; absent = $false; knownInstalledAt = $null }, @{ componentId = $child.id; versionId = $childVersion.id; absent = $false; knownInstalledAt = $null }, @{ componentId = $memberComponent.id; versionId = $memberVersion.id; absent = $false; knownInstalledAt = $null }) } | ConvertTo-Json -Depth 5
$fullWorkbench = Invoke-RestMethod -WebSession $session -Method Post -Uri ([uri]::new($BaseUri, "/api/v1/machines/$($fullWorkbenchMachine.id)/facts")) -Headers $fullWorkbenchHeaders -ContentType 'application/json' -Body $fullWorkbenchBody
$fullWorkbenchReplay = Invoke-RestMethod -WebSession $session -Method Post -Uri ([uri]::new($BaseUri, "/api/v1/machines/$($fullWorkbenchMachine.id)/facts")) -Headers $fullWorkbenchHeaders -ContentType 'application/json' -Body $fullWorkbenchBody
$fullWorkbenchConfiguration = Invoke-RestMethod -WebSession $session -Uri ([uri]::new($BaseUri, "/api/v1/machines/$($fullWorkbenchMachine.id)/configuration"))
$fullWorkbenchFacts = Invoke-RestMethod -WebSession $session -Uri ([uri]::new($BaseUri, "/api/v1/machines/$($fullWorkbenchMachine.id)/facts"))
if ($fullWorkbench.id -ne $fullWorkbenchReplay.id -or @($fullWorkbenchConfiguration | Where-Object { $_.componentId -eq $component.id -and $_.versionId -eq $testingActualVersion.id -and $_.state -eq 'Present' -and (([DateTimeOffset]$_.stateEffectiveAt).ToUniversalTime() - $fullWorkbenchAt.ToUniversalTime()).Duration().TotalSeconds -lt 1 }).Count -ne 1 -or @($fullWorkbenchConfiguration | Where-Object { $_.componentId -eq $child.id -and $_.versionId -eq $childVersion.id -and $_.state -eq 'Present' }).Count -ne 1 -or @($fullWorkbenchFacts | Where-Object { $_.operationType -eq 'InitialSnapshot' -and $_.coverage -eq 'Full' -and $_.sourceType -eq 'manual-ui' -and $_.sourceBaselineId -eq $null }).Count -ne 1) { throw 'Full configuration must record every configured component, permit a Testing version for a laboratory machine, retain the effective time, and remain idempotent without inventing a baseline source.' }
...
$impactExport = Invoke-WebRequest -WebSession $session -Method Post -Uri ([uri]::new($BaseUri, "/api/v1/component-versions/$($secondVersion.id)/impact/export")) -Headers @{ 'Idempotency-Key' = [Guid]::NewGuid().ToString() } -ContentType 'application/json' -Body (@{ reason = '自动化版本影响导出验收' } | ConvertTo-Json)
$impactExportAudit = Invoke-RestMethod -WebSession $session -Uri ([uri]::new($BaseUri, "/api/v1/audit?entityId=$($secondVersion.id)"))
if ($impactExport.StatusCode -ne 200 -or $impactExport.Content -notmatch '当前机台' -or @($impactExportAudit | Where-Object { $_.action -eq 'VersionImpactCsvExported' }).Count -ne 1) { throw 'Version impact export must return a CSV file and record an audit event.' }
...
try { Invoke-JsonPost "/api/v1/baselines/$($oldDraft.id)/maintenance" @{ releasedAt = '2001-01-01T00:00:00Z'; reason = '未发布草稿不能补造发布时间'; maintenanceMode = $true } | Out-Null; throw 'Draft release time maintenance unexpectedly succeeded.' } catch { if ($_.Exception.Message -match 'unexpectedly succeeded') { throw }; if ($_.Exception.Response.StatusCode.value__ -ne 400) { throw } }
...
if (-not [string]::IsNullOrWhiteSpace($ConnectionString)) {
    ...
    $ageFixture = Invoke-TestSql "BEGIN; SET LOCAL confighub.baseline_maintenance = 'on'; UPDATE baseline_lifecycle_transitions SET occurred_at = now() - interval '2 minutes' WHERE configuration_baseline_id = '$($expiredCandidate.id)' AND from_state = 'Draft' AND to_state = 'Released'; COMMIT;"
    ...
    $ageFixture = Invoke-TestSql "BEGIN; SET LOCAL confighub.baseline_maintenance = 'on'; UPDATE baseline_lifecycle_transitions SET occurred_at = now() - interval '4 minutes' WHERE configuration_baseline_id = '$($lateCandidate.id)' AND from_state = 'Draft' AND to_state = 'Released'; COMMIT;"
