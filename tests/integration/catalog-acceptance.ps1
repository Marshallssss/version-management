...
if ($actual.Count -ne 2 -or @($actual | Where-Object { $null -ne $_.knownInstalledAt }).Count -ne 0 -or @($actual | Where-Object { $_.componentId -eq $child.id -and $_.versionId -eq $childVersion.id -and $_.state -eq 'Present' }).Count -ne 1) { throw 'Partial observation must preserve unobserved current components and must not turn observation time into installed time.' }
