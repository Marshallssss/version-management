...
internal static class ConfigHubModel
{
    public static void Configure(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<DeploymentBatch>(entity =>
        {
            ...
            entity.Property(item => item.SourceType).HasColumnName("source_type").HasMaxLength(80); entity.Property(item => item.ExternalEventId).HasColumnName("external_event_id").HasMaxLength(200); entity.Property(item => item.CorrectsDeploymentBatchId).HasColumnName("corrects_deployment_batch_id"); entity.Property(item => item.SourceConfigurationBaselineId).HasColumnName("source_configuration_baseline_id"); entity.Property(item => item.RecordedAt).HasColumnName("recorded_at"); entity.Property(item => item.EffectiveAt).HasColumnName("effective_at");
            ...
            entity.HasIndex(item => item.CorrectsDeploymentBatchId).HasDatabaseName("ix_deployment_batches_corrects_batch"); entity.HasOne<DeploymentBatch>().WithMany().HasForeignKey(item => item.CorrectsDeploymentBatchId).OnDelete(DeleteBehavior.Restrict); entity.HasIndex(item => item.SourceConfigurationBaselineId).HasDatabaseName("ix_deployment_batches_source_baseline"); entity.HasOne<ConfigurationBaseline>().WithMany().HasForeignKey(item => item.SourceConfigurationBaselineId).OnDelete(DeleteBehavior.Restrict);
        });
