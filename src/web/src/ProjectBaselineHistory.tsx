...
import { EditOutlined, FilterOutlined, PlusOutlined, CloseOutlined, HistoryOutlined } from '@ant-design/icons'
...
import { renameBaseline, getMaintenanceCapabilities, assignProjectStandard, compareBaselines, createBaseline, decideBaselineReview, getBaselineDetail, getProjectStandard, maintainBaselineDraft, releaseBaseline, requestBaselineReview, undoBaselineCreation, withdrawBaselineRelease, type ProjectDetail } from './catalog-api'
...
export function ProjectBaselineHistory({ canWrite = true, detail, isAdmin, isSuperAdmin, focusedBaselineId, composerRequest = 0, requestedBaseline, onOpenReference, onOpenPatches, onSuccess }: { canWrite?: boolean; detail: ProjectDetail; isAdmin: boolean; isSuperAdmin: boolean; focusedBaselineId?: string; composerRequest?: number; requestedBaseline: { id: string } | null; onOpenReference: OpenImpactReference; onOpenPatches: (componentId: string, versionId: string) => void; onSuccess: (message: string) => void }) {
  ...
  const chronologicalBaselines = useMemo(() => [...(baselines.data ?? [])].sort((left, right) => (right.releasedAt ? new Date(right.releasedAt).getTime() : 0) - (left.releasedAt ? new Date(left.releasedAt).getTime() : 0) || left.id.localeCompare(right.id)), [baselines.data])
  ...
  const visibleBaselines = useMemo(() => {
    ...
    const result = chronologicalBaselines.filter(baseline => {
      if ((dateFrom || dateTo) && !baseline.releasedAt) return false
      const time = baseline.releasedAt ? new Date(baseline.releasedAt).getTime() : 0
  ...
  useEffect(() => { setOperationAction(null); setRenameOpen(false); setMaintenanceOpen(false); setMaintenanceReason(''); setUndoReason('') }, [selectedBaselineId])
  ...
  const refresh = async () => { await queryClient.invalidateQueries({ queryKey: ['project-baseline-history', detail.project.id] }); await queryClient.invalidateQueries({ queryKey: ['project-standard', detail.project.id] }); await queryClient.invalidateQueries({ queryKey: ['project', detail.project.id] }); await queryClient.invalidateQueries({ queryKey: ['baseline-detail'] }); await queryClient.invalidateQueries({ queryKey: ['baseline-history-comparison'] }); await queryClient.invalidateQueries({ queryKey: ['workspace-project-standard', detail.project.id] }) }
  const rename = useMutation({ mutationFn: () => renameBaseline(selectedBaselineId, newName, renameReason), onSuccess: async () => { setRenameOpen(false); onSuccess('基线名称已更新，冻结快照与发布时间保持不变。'); await refresh(); await queryClient.invalidateQueries({ queryKey: ['machines'] }); await queryClient.invalidateQueries({ queryKey: ['machine-registry'] }) } })
  ...
  const originalReleasedAt = baselineDetail.data?.baseline.releasedAt
  const savedMaintenanceTime = originalReleasedAt && (!maintenanceTime || maintenanceTime === localTime(originalReleasedAt)) ? originalReleasedAt : originalReleasedAt && maintenanceTime ? new Date(maintenanceTime).toISOString() : undefined
  const maintain = useMutation({ mutationFn: () => maintainBaselineDraft(selectedBaselineId, { releasedAt: savedMaintenanceTime, versionSelections: Object.entries(maintenanceSelections).filter(([, versionId]) => versionId).map(([componentId, versionId]) => ({ componentId, versionId })), reason: maintenanceReason, maintenanceMode: true }), onSuccess: async () => { await refresh(); setOperationAction(null); setRenameOpen(false); setMaintenanceOpen(false); setMaintenanceReason(''); onSuccess('历史维护已保存；发布时间和版本快照已更新，实际录入及审计时间保持不变。') } })
  ...
  const undoSecondsRemaining = selected?.releaseRecordedAt ? Math.max(0, 180 - Math.floor((now - new Date(selected.releaseRecordedAt).getTime()) / 1000)) : 0
  ...
  return <section className="project-baseline-history" id="project-baseline-history">
    ...
    <section className="baseline-history-filters" aria-label="基线筛选">
      <div className="baseline-filter-toolbar">
        ...
        <label>发布时间<select aria-label="基线时间排序" value={sortDirection} onChange={event => setSortDirection(event.target.value as 'newest' | 'oldest')}><option value="newest">新到旧</option><option value="oldest">旧到新</option></select></label>
