...
export function RollbackFactPanel({ machineId, components }: { machineId: string; components: ComponentOption[] }) {
  ...
  return <section className="status-panel catalog-panel">
    ...
    <form className="catalog-form" onSubmit={(event) => { event.preventDefault(); rollback.mutate() }}>
      ...
      <label>更正原事实<select value={correctsDeploymentBatchId} onChange={(event) => setCorrectsDeploymentBatchId(event.target.value)}><option value="">不关联：记录回退</option>{facts.data?.filter(fact => fact.operationType !== 'Correction').map(fact => <option key={fact.id} value={fact.id}>{fact.operationType} · {new Date(fact.effectiveAt).toLocaleString('zh-CN')}</option>)}</select></label>
