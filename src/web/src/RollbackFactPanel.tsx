...
type ComponentOption = { id: string; name: string; versions: Array<{ id: string; versionNumber: string; sequenceNo: number }> }
...
export function RollbackFactPanel({ machineId, components }: { machineId: string; components: ComponentOption[] }) {
  ...
  return <section className="status-panel catalog-panel">
    ...
    <form className="catalog-form" onSubmit={(event) => { event.preventDefault(); rollback.mutate() }}>
      <label>组件<select value={componentId} onChange={(event) => { setComponentId(event.target.value); setVersionId('') }} required><option value="">请选择组件</option>{components.map(component => <option key={component.id} value={component.id}>{component.name}</option>)}</select></label>
