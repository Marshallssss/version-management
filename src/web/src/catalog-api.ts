...
export const updateProject = (projectId: string, input: { code: string; name: string; description: string; reason: string }) => request<{ id: string }>(`/api/v1/projects/${projectId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': createIdempotencyKey() }, body: JSON.stringify(input) })
export const renameBaseline = (baselineId: string, name: string, reason: string) => request<{ id: string }>(`/api/v1/baselines/${baselineId}/name`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': createIdempotencyKey() }, body: JSON.stringify({ name, reason }) })
...
export const deleteComponentVersion = (versionId: string, reason: string) => request<{ id: string; deleted: boolean; componentId: string }>(`/api/v1/component-versions/${versionId}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': createIdempotencyKey() }, body: JSON.stringify({ reason }) })
...
export const cloneProject = (projectId: string, input: { code: string; name: string; reason: string; description?: string }) =>
...
export const reorderComponent = (componentId: string, direction: 'Up' | 'Down', reason: string) =>
  request<{ id: string; sortOrder: number }>(`/api/v1/components/${componentId}/reorder`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': createIdempotencyKey() }, body: JSON.stringify({ direction, reason }) })
export const getBaselines = (projectId: string) => request<BaselineSummary[]>(`/api/v1/projects/${projectId}/baselines`)
export const getBaselineDetail = (baselineId: string) => request<{ baseline: { id: string; projectId: string; code: string; seriesCode: string; revisionNo: number; state: string; description: string | null; createdBy: string; createdAt: string; releasedBy: string | null; releasedAt: string | null; releaseRecordedAt: string | null; approvedBy: string | null }; review: { id: string; status: string; requestedBy: string; requestedAt: string; requestReason: string; decidedBy: string | null; decidedAt: string | null; decisionReason: string | null } | null; items: Array<{ id: string; parentItemId: string | null; componentId: string; versionId: string | null; versionNumber: string | null; componentName: string; lineageKey: string; requirement: string; sortOrder: number }> }>(`/api/v1/baselines/${baselineId}`)
...
export const withdrawBaselineRelease = (baselineId: string, reason: string) =>
  request<{ id: string; state: string }>(`/api/v1/baselines/${baselineId}/withdraw-release`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': createIdempotencyKey() }, body: JSON.stringify({ reason }) })
export const maintainBaselineDraft = (baselineId: string, input: { releasedAt?: string; versionSelections?: Array<{ componentId: string; versionId: string }>; reason: string; maintenanceMode: true }) => request<{ id: string; createdAt: string; releasedAt: string | null }>(`/api/v1/baselines/${baselineId}/maintenance`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': createIdempotencyKey() }, body: JSON.stringify(input) })
...
export interface MachineSummary { id: string; projectId: string; serialNumber: string; name: string; machineType: string | null; process: string | null; equipmentConfiguration: string | null; location: string | null; owner: string | null; stage: string | null; chambers: number[]; targetBaselineCode: string | null; expectedResumeAt: string | null; status: string; matchStatus: string | null; riskSeverity: string | null }
export const getMachines = () => request<MachineSummary[]>('/api/v1/machines')
export const createMachine = (input: { projectId: string; serialNumber: string; name: string; machineType: string; process?: string; equipmentConfiguration?: string; location: string; reason: string; owner?: string; stage?: string; chambers?: ChamberInput[] }) => request<{ id: string }>('/api/v1/machines', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': createIdempotencyKey() }, body: JSON.stringify(input) })
export const updateMachine = (machineId: string, input: { serialNumber: string; name: string; machineType: string; process?: string; equipmentConfiguration?: string; location: string; status: string; expectedResumeAt?: string | null; reason: string }) => request<{ id: string }>(`/api/v1/machines/${machineId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': createIdempotencyKey() }, body: JSON.stringify(input) })
...
export const searchCatalog = (query: string, projectId?: string) => request<Array<{ type: string; id: string; projectId: string; versionId?: string; label: string }>>(`/api/v1/search?query=${encodeURIComponent(query)}${projectId ? `&projectId=${encodeURIComponent(projectId)}` : ''}`)
export const getDashboard = (projectId?: string) => request<{ machineCount: number; matchedCount: number; mismatchCount: number; unknownCount: number; criticalRiskCount: number }>(`/api/v1/dashboard${projectId ? `?projectId=${encodeURIComponent(projectId)}` : ''}`)
...
export const recordMachineFacts = (machineId: string, input: { operationType: string; coverage: string; sourceType: string; effectiveAt?: string; reason: string; correctsDeploymentBatchId?: string; items: Array<{ componentId: string; versionId: string | null; absent: boolean; knownInstalledAt: string | null }> }) => request<{ id: string }>(`/api/v1/machines/${machineId}/facts`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': createIdempotencyKey() }, body: JSON.stringify(input) })
...
export const upgradeMachinesToBaseline = (projectId: string, input: { configurationBaselineId: string; machineIds: string[]; effectiveAt: string; reason: string }) => request<{ id: string; baselineId: string; baselineCode: string; effectiveAt: string; succeeded: number; failed: number }>(`/api/v1/projects/${projectId}/bulk-baseline-upgrades`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': createIdempotencyKey() }, body: JSON.stringify(input) })
