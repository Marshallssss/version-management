...
export const createMachine = (input: { projectId: string; serialNumber: string; name: string; machineType: string; reason: string }) => request<{ id: string }>('/api/v1/machines', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify(input) })
...
export const getMachineFacts = (machineId: string) => request<Array<{ id: string; operationType: string; coverage: string; sourceType: string; recordedAt: string; effectiveAt: string; itemCount: number }>>(`/api/v1/machines/${machineId}/facts`)
...
export const assignMachineTarget = (machineId: string, baselineId: string, reason: string) => request<{ id: string; baselineId: string }>(`/api/v1/machines/${machineId}/target`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() }, body: JSON.stringify({ configurationBaselineId: baselineId, reason }) })
