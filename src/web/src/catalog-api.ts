...
export interface MachineSummary { id: string; projectId: string; serialNumber: string; name: string; machineType: string | null; location: string | null; status: string; matchStatus: string | null; riskSeverity: string | null }
...
export const updateMachine = (machineId: string, input: { serialNumber: string; name: string; machineType: string; location: string; status: string; reason: string }) => request<{ id: string }>(`/api/v1/machines/${machineId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': createIdempotencyKey() }, body: JSON.stringify(input) })
