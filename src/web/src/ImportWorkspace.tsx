...
import { createMatrixTemplate, downloadMatrixTemplate, getMatrixWorkspace, saveMatrixSource, scanMatrixSource, scanMatrixWorkbook, workbookBase64, type MatrixCombination, type MatrixComponent, type MatrixRun, type MatrixSource } from './matrix-import-api'
...
export function ImportWorkspace({ projectId, projectName, isAdmin = false, canImport = false, onOpenVersion }: { projectId: string; projectName: string; isAdmin?: boolean; canImport?: boolean; onOpenVersion?: (componentId: string, versionId: string) => void }) {
  ...
  const selectedCombination = combinations.find(item => item.id === selectedCombinationId) ?? combinations[0]
  ...
  const onScanned = async (run: MatrixRun) => {
    ...
    setNotice(hasError(run) ? '' : `扫描完成：新增 ${run.importedCount} 套测试组合，跳过 ${run.skippedCount} 行。`)
    await refresh()
  }
  ...
  return <section className="status-panel catalog-panel import-workspace matrix-import-workspace">
    ...
    {canImport ? <form className="matrix-upload-form" aria-label="Excel 扫描录入" onSubmit={event => { event.preventDefault(); setNotice(''); scan.mutate() }}>
      ...
      {activeTemplate && <div className="matrix-semantic-note"><span>{activeTemplate.componentCount} 个组件</span><span>新版本仅登记为测试中</span><span>仅提交“可导入”行；空白继承上一套组合</span></div>}
    ...
    {tab === 'combinations' && (combinations.length ? <div className="matrix-history-layout"><div className="matrix-history-list" aria-label="测试组合列表">{combinations.map(item => <button type="button" key={item.id} aria-pressed={selectedCombination?.id === item.id} onClick={() => setSelectedCombinationId(item.id)}><strong>第 {item.sequenceNo} 套 <span>{item.items.filter(component => component.changed).length} 项变更</span></strong><small>{item.recordDate} · Excel 第 {item.sourceRow} 行</small><span>{item.reason}</span></button>)}</div>{selectedCombination && <CombinationTree combination={selectedCombination} onOpenVersion={onOpenVersion} />}</div> : <p className="empty-state">尚无测试组合。填写项目模板并扫描后，组合记录将显示在这里。</p>)}
    {tab === 'runs' && (runs.length || latestRun ? <div className="matrix-history-layout"><div className="matrix-history-list" aria-label="扫描记录列表">{(latestRun && !runs.some(item => item.id === latestRun.id) ? [latestRun, ...runs] : runs).map(run => <button type="button" key={run.id} aria-pressed={selectedRun?.id === run.id} onClick={() => setSelectedRunId(run.id)}><strong>{run.fileName || '固定文件扫描'}<span className={hasError(run) ? 'matrix-error-text' : ''}>{stateText(run.status)}</span></strong><small>{formatTime(run.createdAt)}</small><span>新增 {run.importedCount} 套 · 跳过 {run.skippedCount} 行</span></button>)}</div>{selectedRun && <RunResult run={selectedRun} />}</div> : <p className="empty-state">尚无扫描记录。</p>)}
...
function CombinationTree({ combination, onOpenVersion }: { combination: MatrixCombination; onOpenVersion?: (componentId: string, versionId: string) => void }) {
  ...
  const renderNode = (item: MatrixComponent, depth: number, path: Set<string>): ReactNode => {
    ...
    return <div key={item.componentId} className="matrix-tree-branch"><div className={`matrix-tree-node${item.changed ? ' changed' : ''}${depth === 0 ? ' root' : ''}`}><span>{item.componentName}</span>{item.versionId && onOpenVersion ? <button className="matrix-version-link" type="button" onClick={() => onOpenVersion(item.componentId, item.versionId!)}>{item.versionNumber}</button> : <strong>{item.isCategory ? '分类' : item.versionNumber || '尚未指定'}</strong>}{item.changed && <small>变更</small>}</div>{children(item.componentId).map(child => renderNode(child, depth + 1, nextPath))}</div>
  }
