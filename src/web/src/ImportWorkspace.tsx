...
export function ImportWorkspace({ projectId, projectName }: { projectId: string; projectName: string }) {
  ...
  return <section className="status-panel catalog-panel import-workspace">
    <div className="panel-heading"><div><span className="section-index">{projectName}</span><h3><ImportOutlined aria-hidden />导入预览</h3></div>{preview.data && <span className="import-count">{preview.data.rows.length} 条记录</span>}</div>
