        ...
        {selectedNavigation.available ? (
          <div className="content-grid">
            ...
            {activePage === 'projects' && <>
              ...
              {projectDetail.data && <ProjectWorkspace detail={projectDetail.data} focusedVersionId={focusedVersionId} onSuccess={setSuccessMessage} />}
            ...
            {activePage === 'search' && <section className="status-panel catalog-panel"><div className="panel-heading"><div><span className="section-index">全局搜索</span><h3>项目、组件、版本、补丁、基线和机台</h3></div></div><label>搜索词<input value={searchTerm} minLength={2} onChange={(event) => setSearchTerm(event.target.value)} placeholder="至少输入两个字符" /></label>{searchTerm.trim().length >= 2 && <div className="catalog-list">{catalogSearch.data?.map((item) => <button type="button" className="component-row" key={`${item.type}-${item.id}`} onClick={() => { if (item.type === 'Project' || item.type === 'Component' || item.type === 'Version' || item.type === 'Patch') { setSelectedProjectId(item.projectId); setFocusedVersionId(item.type === 'Patch' ? item.versionId ?? '' : item.type === 'Version' ? item.id : ''); setActivePage('projects') } else if (item.type === 'Baseline') { setBaselineProjectId(item.projectId); setSelectedBaselineId(item.id); setActivePage('baselines') } else { setSelectedMachineId(item.id); setActivePage('machines') } }}><div><strong>{item.label}</strong><span>{item.type === 'Project' ? '项目' : item.type === 'Component' ? '组件' : item.type === 'Version' ? '版本' : item.type === 'Patch' ? '补丁' : item.type === 'Baseline' ? '基线' : '机台'}</span></div><small>打开</small></button>)}</div>}{catalogSearch.isError && <p className="error-strip">{catalogSearch.error.message}</p>}</section>}
