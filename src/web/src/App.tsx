...
function App() {
  ...
  const [projectDialog, setProjectDialog] = useState<'login' | 'switch' | 'create' | 'edit' | 'account' | null>(null)
  ...
  const addProject = useMutation({
    mutationFn: (input: { code: string; name: string; description: string; reason: string }) => cloneSourceProjectId ? cloneProject(cloneSourceProjectId, { code: input.code, name: input.name, description: input.description, reason: input.reason }) : createProject(input),
    onSuccess: async ({ id }) => {
      setProjectCode('')
      setProjectName('')
  ...
  const editProject = useMutation({ mutationFn: () => updateProject(selectedProjectId!, { code: projectCode, name: projectName, description: projectDescription, reason: projectReason }), onSuccess: async () => { setProjectDialog(null); setProjectReason(''); setSuccessMessage('项目资料已保存。'); await queryClient.invalidateQueries({ queryKey: ['projects'] }); await queryClient.invalidateQueries({ queryKey: ['project', selectedProjectId] }) } })
      ...
      <main>
        ...
        {selectedNavigation.available ? (
          <div className="content-grid">
            ...
            {activePage === 'projects' && <>
              ...
              <section className="status-panel catalog-panel">
                ...
                <form className="catalog-form" onSubmit={(event) => { event.preventDefault(); addProject.mutate({ code: projectCode, name: projectName, description: projectDescription, reason: projectReason }) }}>
                  ...
                  <label className="wide-field">创建方式<select value={cloneSourceProjectId} onChange={(event) => { setCloneSourceProjectId(event.target.value); if (event.target.value) setProjectDescription(projects.data?.find(project => project.id === event.target.value)?.description ?? '') }}><option value="">创建空白项目</option>{projects.data?.map((project) => <option key={project.id} value={project.id}>从 {project.code} · {project.name} 复制组件树</option>)}</select></label>
                  {cloneSourceProjectId && <p className="form-hint wide-field">将复制组件树，不复制软件版本、基线、机台或历史记录。</p>}<label className="wide-field">说明<textarea value={projectDescription} maxLength={2000} onChange={(event) => setProjectDescription(event.target.value)} /></label>
            ...
            {activePage === 'imports' && canWrite && (selectedProjectId ? <ImportWorkspace key={selectedProjectId} projectId={selectedProjectId} projectName={selectedProject?.name ?? ''} onOpenVersion={(componentId, versionId) => { setFocusedComponentId(componentId); setFocusedVersionId(versionId); setFocusPatch(false); setActivePage('projects') }} isAdmin={isAdmin} canImport={currentUser.data?.roles.some(role => ['SeniorEngineer', 'Admin', 'SuperAdmin'].includes(role)) === true} /> : <p className="empty-state">尚未选择项目。</p>)}
