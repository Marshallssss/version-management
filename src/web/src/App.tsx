...
function App() {
  ...
  const stageImportMutation = useMutation({ mutationFn: () => stageImport({ projectId: importProjectId, sourceFileName: '手工预览.csv', reason: importReason, rows: parseImportRows(importRows) }), onSuccess: ({ id }) => setImportBatchId(id) })
      ...
      <aside className="rail">
        ...
        <nav aria-label="主导航">
          {visibleNavigation.map((item, index) => (
            <button className={item.id === activePage ? 'nav-item active' : 'nav-item'} key={item.id} type="button" onClick={() => { if (item.id === 'machines') setSelectedMachineId(''); setActivePage(!isAuthenticated && item.id !== 'projects' ? 'projects' : item.id) }}>
        ...
        {selectedNavigation.available ? (
          <div className="content-grid">
            ...
            {activePage === 'projects' && <>
              ...
              {isAuthenticated && selectedProjectId && projectDetail.data && <ProjectWorkspace canWrite={canWrite} detail={projectDetail.data} focusedVersionId={focusedVersionId} focusedBaselineId={focusedBaselineId} isAdmin={isAdmin} isSuperAdmin={isSuperAdmin} onSuccess={setSuccessMessage} />}
              <div hidden>
            ...
            {activePage === 'search' && <section className="status-panel catalog-panel"><div className="panel-heading"><div><span className="section-index">全局搜索</span><h3>项目、组件、版本、补丁、基线和机台</h3></div></div><label>搜索词<input value={searchTerm} minLength={2} onChange={(event) => setSearchTerm(event.target.value)} placeholder="至少输入两个字符" /></label>{searchTerm.trim().length >= 2 && <div className="catalog-list">{catalogSearch.data?.map((item) => <button type="button" className="component-row" key={`${item.type}-${item.id}`} onClick={() => { if (item.type === 'Project' || item.type === 'Component' || item.type === 'Version' || item.type === 'Patch') { setSelectedProjectId(item.projectId); localStorage.setItem('confighub.selected-project-id', item.projectId); setFocusedBaselineId(''); setFocusedVersionId(item.type === 'Patch' ? item.versionId ?? '' : item.type === 'Version' ? item.id : ''); setActivePage('projects') } else if (item.type === 'Baseline') { setSelectedProjectId(item.projectId); localStorage.setItem('confighub.selected-project-id', item.projectId); setFocusedVersionId(''); setFocusedBaselineId(item.id); setActivePage('projects') } else { setSelectedMachineId(item.id); setActivePage('machines') } }}><div><strong>{item.label}</strong><span>{item.type === 'Project' ? '项目' : item.type === 'Component' ? '组件' : item.type === 'Version' ? '版本' : item.type === 'Patch' ? '补丁' : item.type === 'Baseline' ? '基线' : '机台'}</span></div><small>打开</small></button>)}</div>}{catalogSearch.isError && <p className="error-strip">{catalogSearch.error.message}</p>}</section>}

            {activePage === 'compare' && <ConfigurationCompare key={selectedProjectId ?? ''} projectId={selectedProjectId ?? ''} />}
            ...
            {activePage === 'machines' && <MachineWorkspace key={selectedProjectId ?? ''} projectId={selectedProjectId ?? ''} canWrite={canWrite} isAdmin={isAdmin} selectedMachineId={selectedMachineId} onSelectMachine={setSelectedMachineId} onOpenVersion={(projectId, versionId) => { setSelectedProjectId(projectId); localStorage.setItem('confighub.selected-project-id', projectId); setFocusedBaselineId(''); setFocusedVersionId(versionId); setActivePage('projects') }} onSuccess={setSuccessMessage} />}
          </div>
        ) : (
