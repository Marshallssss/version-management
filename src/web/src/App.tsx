...
function App() {
  ...
  const addComponent = useMutation({
    mutationFn: ({ projectId, code, name, reason }: { projectId: string; code: string; name: string; reason: string }) => createComponent(projectId, { code, name, parentComponentId: null, reason }),
    onSuccess: async () => {
      setComponentCode('')
      setComponentName('')
      setComponentReason('')
      ...
      <main>
        ...
        {selectedNavigation.available ? (
          <div className="content-grid">
            ...
            {activePage === 'projects' && <>
              ...
              {projectDetail.data && <section className="status-panel catalog-detail">
                ...
                <div className="catalog-actions">
                  <form className="inline-form" onSubmit={(event) => { event.preventDefault(); addComponent.mutate({ projectId: projectDetail.data.project.id, code: componentCode, name: componentName, reason: componentReason }) }}><label>组件编码<input value={componentCode} placeholder="例如：PLC" onChange={(event) => setComponentCode(event.target.value)} required /></label><label>组件名称<input value={componentName} placeholder="例如：主控程序" onChange={(event) => setComponentName(event.target.value)} required /></label><label>创建原因<input value={componentReason} onChange={(event) => setComponentReason(event.target.value)} required /></label><button type="submit" disabled={addComponent.isPending}>{addComponent.isPending ? '正在新增' : '新增组件'}</button></form>
            ...
            {activePage === 'users' && <section className="status-panel catalog-panel"><div className="panel-heading"><div><span className="section-index">身份管理</span><h3>用户与角色</h3></div><span className="count">{users.data?.length ?? 0}</span></div>{currentUser.data?.roles.includes('Admin') ? <div className="catalog-list">{users.data?.map(user => <article className="component-row" key={user.id}><div><strong>{user.displayName}</strong><span>{user.email}</span></div><small>{user.roles.join('、') || '未分配角色'}</small></article>)}</div> : <p className="empty-state">仅管理员可查看用户与角色。</p>}{users.isError && <p className="error-strip">{users.error.message}</p>}</section>}
