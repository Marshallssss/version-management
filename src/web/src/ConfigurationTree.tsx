...
export function ConfigurationTree({ components, items, onOpenVersion, onOpenPatches }: { components: ConfigurationComponent[]; items: ActualItem[]; onOpenVersion: (versionId: string) => void; onOpenPatches: (versionId: string) => void }) {
  ...
  const node = (component: ConfigurationComponent, root: boolean): ReactNode => {
    ...
    return <div key={component.id} className={root ? 'actual-root-column' : 'actual-child-branch'}>
      <div className="actual-node-row"><button type="button" className={`actual-node${root ? ' actual-root' : ''}${item?.state === 'Absent' ? ' absent' : ''}`} disabled={!item?.versionId} onClick={() => item?.versionId && onOpenVersion(item.versionId)} title={`${component.name}\n${versionLabel(item, component)}${item ? `\n状态生效：${new Date(item.stateEffectiveAt).toLocaleString('zh-CN')}\n已知安装：${item.knownInstalledAt ? new Date(item.knownInstalledAt).toLocaleString('zh-CN') : '未知'}` : ''}`}>
        <strong>{component.name}</strong><span>{versionLabel(item, component)}</span>
      </button>{item?.state === 'Present' && item.versionId && <PatchBadge context="machine" version={component.versions.find(version => version.id === item.versionId)} onOpen={() => onOpenPatches(item.versionId!)} />}</div>
