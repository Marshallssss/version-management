...
import { ImportOutlined, FileSearchOutlined, CheckOutlined } from '@ant-design/icons'

export function LegacyImportWorkspace({ projectId, projectName }: { projectId: string; projectName: string }) {
  ...
  return <section className="legacy-import-workspace">
    <div className="panel-heading"><div><h3><ImportOutlined aria-hidden />文本导入预览</h3></div>{preview.data && <span className="import-count">{preview.data.rows.length} 条记录</span>}</div>
    <form className="catalog-form" onSubmit={event => { event.preventDefault(); setBatchId(''); commit.reset(); stage.mutate() }}>
      ...
      <div className="form-actions wide-field"><button className="primary-action" type="submit" disabled={busy}><FileSearchOutlined aria-hidden />{stage.isPending ? '正在校验' : '生成预览'}</button></div>
    </form>
    {batchId && preview.isLoading && <p role="status">正在读取导入预览。</p>}
    {preview.data && <div className="component-list import-preview-list">{preview.data.rows.map(row => <article className={`component-row${row.validationError ? ' import-row-invalid' : ''}`} key={row.rowNumber}><div><strong>第 {row.rowNumber} 行</strong><span>{row.payload.componentName} · {row.payload.versionNumber}</span></div><small>{row.validationError ?? (preview.data.status === 'Committed' ? '已导入' : '校验通过，尚未提交')}</small></article>)}</div>}
    {preview.data?.status === 'Validated' && !preview.data.rows.some(row => row.validationError) && !commit.isSuccess && <button type="button" className="primary-action" disabled={busy} onClick={() => commit.mutate()}><CheckOutlined aria-hidden />{commit.isPending ? '正在提交' : '提交导入'}</button>}
