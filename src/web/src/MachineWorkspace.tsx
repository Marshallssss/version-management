...
import { PlusOutlined, EditOutlined, ArrowLeftOutlined, HistoryOutlined, SettingOutlined, SwapOutlined, UnorderedListOutlined } from '@ant-design/icons'
...
export function MachineWorkspace({ projectId, canWrite = true, isAdmin = false, selectedMachineId, onSelectMachine, onOpenVersion, onSuccess }: { projectId: string; canWrite?: boolean; isAdmin?: boolean; selectedMachineId: string; onSelectMachine: (machineId: string) => void; onOpenVersion: (projectId: string, versionId: string) => void; onSuccess: (message: string) => void }) {
  ...
  return <div className={`machine-workspace${selectedMachine ? ' has-selection' : ' machine-unselected'}`}>
    <section className="machine-registry-panel">
      <div className="panel-heading machine-heading"><strong>机台管理 <span className="workspace-count">{visibleMachines.length} 台</span></strong><div className="toolbar-actions">{canWrite && projectId && <><button type="button" aria-label="新建机台 / 从已有机台复制资料" onClick={() => setCreateOpen(true)}><PlusOutlined />新建机台</button><button type="button" onClick={() => setToolTab('upgrade')}>批量升级</button><button type="button" onClick={() => setToolTab('target')}>批量目标</button></>}</div></div>
    </section>
    ...
    <section hidden={!selectedMachine} className="machine-detail-panel">
      {selectedMachine && <>
        ...
        <div className="machine-statuses"><span>{machineStatusText[selectedMachine.status]}{selectedMachine.expectedResumeAt && ` · 预计恢复 ${formatTime(selectedMachine.expectedResumeAt)}`}</span><span>匹配 {matchText[machineDrift.data?.matchStatus ?? 'Unknown']}</span><span>风险 {riskText[machineDrift.data?.riskSeverity ?? 'Unknown']}</span><span>阶段 {selectedMachine.stage ?? '未登记'}</span><span>负责人 {selectedMachine.owner || '未填写'}</span></div>
        <nav className="workspace-tabs" aria-label="机台详情">
          {[['configuration', '当前配置', <UnorderedListOutlined />], ['target', '目标与对比', <SwapOutlined />], ['equipment', '阶段与腔室', <SettingOutlined />], ['history', '历史', <HistoryOutlined />]].map(([id, label, icon]) => <button type="button" key={String(id)} aria-pressed={detailTab === id} onClick={() => setDetailTab(String(id))}>{icon}{label}</button>)}
        </nav>
        ...
        <div hidden={detailTab !== 'history'} className="machine-section machine-detail-sections">
          ...
          <HistoricalConfigurationPanel key={selectedMachine.id} machine={selectedMachine} />
        </div>
      </>}
    </section>
