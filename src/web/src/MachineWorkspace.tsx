...
export function MachineWorkspace({ projectId, canWrite = true, selectedMachineId, onSelectMachine, onOpenVersion, onSuccess }: { projectId: string; canWrite?: boolean; selectedMachineId: string; onSelectMachine: (machineId: string) => void; onOpenVersion: (projectId: string, versionId: string) => void; onSuccess: (message: string) => void }) {
  ...
  return <div className="machine-workspace">
    ...
    <section className="status-panel catalog-panel machine-detail-panel">
      {!selectedMachine ? <p className="empty-state">从左侧列表选择一台机台后，才会显示其实际配置、目标与历史。</p> : <>
        ...
        <div className="machine-detail-sections">
          ...
          <details open><summary>实际配置</summary><form hidden={!canWrite} className="catalog-form" onSubmit={(event) => { event.preventDefault(); recordFacts.mutate() }}><label>组件<select value={factComponentId} onChange={(event) => { setFactComponentId(event.target.value); setFactVersionId('') }} required><option value="">请选择组件</option>{machineProject.data?.components.filter(component => component.versions.length > 0).map(component => <option key={component.id} value={component.id}>{component.name}</option>)}</select></label><label>版本<select value={factVersionId} onChange={(event) => setFactVersionId(event.target.value)} disabled={!selectedFactComponent} required><option value="">{selectedFactComponent ? '请选择版本' : '请先选择组件'}</option>{selectedFactComponent?.versions.map(version => <option key={version.id} value={version.id}>{version.versionNumber} · {version.maturity === 'Testing' ? '实验室测试' : version.maturity}</option>)}</select></label><label>观察原因<input value={factReason} onChange={(event) => setFactReason(event.target.value)} required /></label><button type="submit" disabled={recordFacts.isPending || machineProject.isLoading}>{recordFacts.isPending ? '正在记录' : '记录局部观察'}</button></form><p className="form-hint">这里只记录一个组件的局部观察，不会改动其他组件。实验室机台可选择测试中的版本，但它不能成为机台目标基线；需要完整配置请使用下方工作台。</p>{recordFacts.isError && <p className="error-strip">{recordFacts.error.message}</p>}<div className="component-list">{machineConfiguration.data?.map(item => <article className="component-row" key={item.componentId}><div><strong>{item.componentName}</strong><span>{item.state === 'Present' ? '当前存在' : '当前缺失'}</span></div><div className="version-tags">{item.versionId ? <button type="button" className="configuration-version-link" onClick={() => onOpenVersion(selectedMachine.projectId, item.versionId!)}>{item.versionNumber}<small>打开版本详情<br />状态生效 {formatTime(item.stateEffectiveAt)}<br />已知安装 {formatTime(item.knownInstalledAt)}</small></button> : <span>无版本<small>状态生效 {formatTime(item.stateEffectiveAt)}<br />已知安装 {formatTime(item.knownInstalledAt)}</small></span>}</div></article>)}</div></details>
    ...
    <div className="machine-tools">
      ...
      {canWrite && selectedMachine && toolTab === 'configuration' && <FullConfigurationPanel machineId={selectedMachine.id} projectId={selectedMachine.projectId} components={machineProject.data?.components ?? []} onRecorded={() => onSuccess('完整实际配置已记录为可追溯事实。')} />}
      {canWrite && selectedMachine && toolTab === 'correction' && <RollbackFactPanel machineId={selectedMachine.id} components={machineProject.data?.components ?? []} />}
      {selectedMachine && toolTab === 'history' && <HistoricalConfigurationPanel machine={selectedMachine} />}
      {canWrite && toolTab === 'upgrade' && <BulkBaselineUpgradePanel projectId={projectId} />}
      {canWrite && toolTab === 'target' && <BulkTargetPanel projectId={projectId} />}
    </div>
  </div>
}
