...
import { ExperimentOutlined, DeploymentUnitOutlined, ArrowLeftOutlined, ArrowRightOutlined, HistoryOutlined, PlusOutlined, SortAscendingOutlined, MenuFoldOutlined, MenuUnfoldOutlined } from '@ant-design/icons'
...
export function ProjectWorkspace({ canWrite = true, detail, focusedVersionId, focusedComponentId, focusPatch, isAdmin, isSuperAdmin, onOpenMachine, onOpenBaseline, onCreateBaseline, onSuccess }: { canWrite?: boolean; detail: ProjectDetail; focusedVersionId?: string; focusedComponentId?: string; focusPatch?: boolean; onOpenMachine?: (machineId: string) => void; onOpenBaseline: (baselineId: string) => void; onCreateBaseline: () => void; isAdmin: boolean; isSuperAdmin: boolean; onSuccess: (message: string) => void }) {
  ...
  const renderTestingNode = (component: ConfigurationComponent, depth: number) => <div className="lab-branch-node" key={component.id} style={{ paddingLeft: `${depth * 12}px` }}>
    <div className={`lab-node-shell${testingVersions.get(component.id)?.length ? ' has-lab-status' : ''}`}>
  ...
  const versionSelection = selectedVersion ? <div className="selected-version-summary"><strong>{selectedVersion.versionNumber}</strong><small>{maturityText(selectedVersion.maturity)} · {safetyText(selectedVersion.safety)}</small><button type="button" className="laboratory-badge" onClick={() => setLaboratoryVersionId(selectedVersion.id)}><ExperimentOutlined aria-hidden />Lab 记录</button>{isAdmin && <DeleteVersionButton onOpenReference={openReference} version={selectedVersion} componentName={selected?.name ?? ''} onDeleted={async () => { setSelectedVersionId(''); setInspectorTab('versions'); await refresh(); onSuccess('版本已删除。') }} />}</div> : <p className="empty-state">先在“版本”中选择或登记软件版本。</p>
