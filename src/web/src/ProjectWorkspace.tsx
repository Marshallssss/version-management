...
export function ProjectWorkspace({ detail, focusedVersionId, onSuccess }: { detail: ProjectDetail; focusedVersionId?: string; onSuccess: (message: string) => void }) {
  ...
  const testingBranchIds = useMemo(() => {
    const ids = new Set<string>()
    const markTestingBranch = (component: ConfigurationComponent) => {
      const hasTestingVersion = (testingVersions.get(component.id)?.length ?? 0) > 0
      const hasTestingDescendant = (children.get(component.id) ?? []).some(markTestingBranch)
      if (hasTestingVersion || hasTestingDescendant) ids.add(component.id)
      return hasTestingVersion || hasTestingDescendant
    }
    for (const root of children.get(null) ?? []) markTestingBranch(root)
    return ids
  ...
  const addVersion = useMutation({
    ...
    onSuccess: async ({ id }) => { setVersionNumber(''); setVersionReason(''); setInitialMaturity('Testing'); setSelectedVersionId(id); onSuccess(`${maturityText(initialMaturity)}版本已登记。`); await refresh() },
  })
  ...
  return <section className="project-workspace">
    ...
    <div className="workspace-layout">
      <aside className="component-tree-panel">
        <section className="laboratory-tree"><div className="tree-toolbar"><strong>实验室测试版本</strong><small>根组件完整展示；子组件只在自身或后代存在“测试中”版本时显示。发布后自动从此处移除，并可在创建新基线时冻结。</small></div><div className="lab-root-grid">{children.get(null)?.map(root => <section className="lab-root-column" key={root.id}>{renderTestingNode(root, 0)}</section>)}</div></section>
