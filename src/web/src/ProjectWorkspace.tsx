...
export function ProjectWorkspace({ detail, focusedVersionId, onSuccess }: { detail: ProjectDetail; focusedVersionId?: string; onSuccess: (message: string) => void }) {
  ...
  const [initialMaturity, setInitialMaturity] = useState<'Draft' | 'Testing'>('Testing')
  ...
  const addVersion = useMutation({
    ...
    onSuccess: async ({ id }) => { setVersionNumber(''); setVersionReason(''); setInitialMaturity('Testing'); setSelectedVersionId(id); onSuccess(initialMaturity === 'Testing' ? '实验室测试版本已登记。' : '草稿版本已登记。'); await refresh() },
  })
  ...
  const renderBranchNode = (component: ConfigurationComponent, depth: number) => <div className="branch-node" key={component.id} style={{ paddingLeft: `${depth * 12}px` }}>
    <button type="button" draggable className={`tree-node ${component.id === selectedId ? 'selected' : ''}`} onClick={() => setSelectedId(component.id)} onDragStart={(event) => { event.dataTransfer.effectAllowed = 'move'; setDraggingId(component.id) }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); onDrop(component.id) }}>
      ...
      <span className="tree-node-copy"><strong>{component.name}</strong><small>{component.versions.length ? '可登记软件版本' : '结构分类节点（暂无版本）'}</small><small className="baseline-version">标准基线：{standardVersionText(component)}</small></span>
  ...
  const renderTestingNode = (component: ConfigurationComponent, depth: number) => <div className="lab-branch-node" key={component.id} style={{ paddingLeft: `${depth * 12}px` }}>
    <button type="button" className={component.id === selectedId ? 'lab-node selected' : 'lab-node'} onClick={() => setSelectedId(component.id)}>
      <span><strong>{component.name}</strong><small>{testingVersions.get(component.id)?.length ? testingVersions.get(component.id)!.map(version => version.versionNumber).join('、') : '暂无实验室测试版本'}</small></span>
      <span>{testingVersions.get(component.id)?.length ?? 0}</span>
    </button>
    {children.get(component.id)?.map(child => renderTestingNode(child, depth + 1))}
  </div>
  ...
  return <section className="project-workspace">
    ...
    <div className="workspace-layout">
      <aside className="component-tree-panel">
        <div className="tree-toolbar"><strong>当前基线版本</strong><small>{projectStandard.data ? `项目标准：${projectStandard.data.baselineCode}；仅已发布版本可进入新基线` : '尚未设置项目标准；仅已发布版本可进入新基线'}</small></div>
        ...
        <section className="laboratory-tree"><div className="tree-toolbar"><strong>实验室测试版本</strong><small>根节点完整展示；只有“测试中”的版本会自动出现。发布后自动从此处移除，并可在创建新基线时冻结。</small></div><div className="lab-root-grid">{children.get(null)?.map(root => <section className="lab-root-column" key={root.id}>{renderTestingNode(root, 0)}</section>)}</div></section>
      </aside>
      <section className="component-inspector">
        {selected ? <><div className="inspector-heading"><div><span className="section-index">已选组件</span><h3>{selected.name}</h3></div><div className="inspector-actions"><button type="button" onClick={startEdit}>编辑</button><button type="button" className="danger-action" onClick={() => { setFormMode('delete'); setReason('') }}>删除</button></div></div>
          ...
          <section className="version-workbench"><div className="subsection-heading"><div><span className="section-index">软件版本</span><h3>登记与状态</h3><p className="form-hint">测试中版本会自动显示在实验室测试树；发布后才可进入新基线，项目标准和机台目标仍需显式操作。</p></div></div><form className="workspace-form" onSubmit={(event) => { event.preventDefault(); addVersion.mutate() }}><label>版本号<input value={versionNumber} placeholder="例如：2026.09.01" maxLength={160} onChange={(event) => setVersionNumber(event.target.value)} required /></label><label>登记状态<select value={initialMaturity} onChange={(event) => setInitialMaturity(event.target.value as 'Draft' | 'Testing')}><option value="Testing">实验室测试</option><option value="Draft">草稿</option></select></label><label className="wide-field">登记原因<input value={versionReason} maxLength={500} onChange={(event) => setVersionReason(event.target.value)} required /></label><div className="form-actions wide-field"><button className="primary-action" type="submit" disabled={addVersion.isPending}>{addVersion.isPending ? '正在登记' : initialMaturity === 'Testing' ? '登记实验室测试版本' : '登记草稿版本'}</button></div>{addVersion.isError && <p className="error-strip wide-field">{addVersion.error.message}</p>}</form>
