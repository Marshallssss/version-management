...
Write-Host "Release $($manifest.product) $($manifest.version) passed manifest verification."
...
try {
    Write-Host 'Phase 1/5: entering maintenance and stopping ConfigHub services.'
    ...
    Write-Host 'Phase 2/5: creating and verifying the quiesced backup.'
    ...
    Write-Host "Phase 3/5: copying current Host and Worker binaries to $rollbackPath."
    ...
    Write-Host 'Phase 4/5: applying the verified release package.'
    ...
    Write-Host 'Phase 5/5: applying forward-only database migrations.'
    ...
    if ($maintenanceEntered) { Exit-ConfigHubMaintenance -HostPath $paths.Host }
...
if ($upgradeSucceeded) {
    ...
    Write-Host 'Run health-check.ps1 against the HTTPS URL before ending the maintenance window.'
}
